...
public interface PetitionRepo extends JpaRepository<Petition, Integer> {
    ...
    @Query("SELECT new com.youngyoung.server.mora.dto.PetitionRes$PetitionInfo(" +
            "p.title, p.type, p.petitionNeeds, p.petitionSummary, p.status, p.category, p.voteStartDate, p.voteEndDate, p.result, p.positiveEx, p.negativeEx, p.good, p.bad, p.allows) " +
            "FROM Petition p WHERE "+
            "p.id =:id")
